...
module.exports.getStandingsByYearAndTeam = async function(team_id, year_id) {
    return await Standings.findOne({ team: team_id, year: year_id }).populate('year').select('-_id');
}
...
module.exports.getStandingsByTeam = async function(team_id) {
    return await Standings.find({ team: team_id }).populate('year').select('-_id');
}
...
module.exports.getStandingsByYear = async function(year_id) {
    return await Standings.find({ year: year_id }).populate('year').select('-_id');
}
...
module.exports.getAllStandings = async function() {
    return await Standings.find({}).populate('year').select('-_id');
}